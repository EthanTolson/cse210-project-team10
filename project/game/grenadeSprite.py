...
class GrenadeSprite(arcade.Sprite):
    ...
    def update(self):
        super().update()
        self.angle += 6
        if self.time + 1 < time.time() and self.check < 3:
            for i in range(0, 10):
                ...
                x = math.cos( math.atan2(10, 10) + (36 * i * math.pi / 180)) * math.sqrt((10)**2 + (10)**2) + self.center_x
                y = math.sin( math.atan2(10, 10) + (36 * i * math.pi / 180)) * math.sqrt((10)**2 + (10)**2) + self.center_y

                projectile.change_x = 20 * ((x - self.center_x ) / math.sqrt((x-self.center_x)**2 + (y- self.center_y)**2))
                projectile.change_y = 20 * ((y - self.center_y ) / math.sqrt((x-self.center_x)**2 + (y- self.center_y)**2))
